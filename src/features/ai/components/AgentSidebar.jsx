...
const AgentSidebar = ({ onInsertContent, onAddReference, sessionId, modelProvider, setModelProvider }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            // Use 'localhost' to match typical browser origin expectations if the app is served from localhost
            // or use relative path if proxy is set up. 
            // The safest is usually valid absolute URL matching the server.
            ...
            // Direct Markdown Insertion: Pass raw markdown to parent
            if (onInsertContent) onInsertContent(accumulatedText);
    ...
    // Helper for manual insert button
    const handleManualInsert = async () => {
        if (!streamingContent || !onInsertContent) return;
        onInsertContent(streamingContent);
    };
        ...
        <div className="agent-sidebar">
                ...
                <div className="sidebar-content scrollable-content">
                    ...
                    <div className="command-area">
                        ...
                        <div className="action-buttons">
                            {isProcessing ? (
                                <button
                                    className="generate-btn loading-gradient"
                                    disabled
                                    style={{ cursor: 'wait', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                                >
                                ...
                                <button
                                    ...
                                    disabled={!command.trim() || !modelProvider}
                                >
                        ...
                        {/* Live Preview Removed as per user request for direct format */}
                        {isProcessing && (
            ...
            <div className="model-selector-footer" style={{
                ...
                <div className="custom-select-wrapper" style={{ position: 'relative' }}>
                    ...
                    >
                        <option value="gemini">Google Gemini 2.0 {modelStatuses['gemini'] && modelStatuses['gemini'] !== 'ok' && '(Limited)'}</option>
                        <option value="openai">OpenAI GPT-4o {modelStatuses['openai'] && modelStatuses['openai'] !== 'ok' && '(Limited)'}</option>
                        <option value="deepseek">DeepSeek V3 {modelStatuses['deepseek'] && modelStatuses['deepseek'] !== 'ok' && '(Limited)'}</option>
                        <option value="llama">Llama 3 (Groq) {modelStatuses['llama'] && modelStatuses['llama'] !== 'ok' && '(Limited)'}</option>
                        <option value="grok">Grok (xAI) {modelStatuses['grok'] && modelStatuses['grok'] !== 'ok' && '(Limited)'}</option>
                        <option value="zhipu">Zhipu AI (GLM-4) {modelStatuses['zhipu'] && modelStatuses['zhipu'] !== 'ok' && '(Limited)'}</option>
                        <option value="ollama">Ollama (Local) {modelStatuses['ollama'] && modelStatuses['ollama'] !== 'ok' && '(Offline)'}</option>
